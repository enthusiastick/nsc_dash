...
SCHEDULER.every '1m', :first_in => 0 do
  ...
  dates1.first.Contacts.each do |contact|
    unless contact.Birthdate.nil?
      if Date.parse(contact.Birthdate).month == today.month
        if special_days.key?(Date.parse(contact.Birthdate))
          special_days[Date.parse(contact.Birthdate)] << "#{contact.Name} Birthday"
        else
          special_days[Date.parse(contact.Birthdate)] = ["#{contact.Name} Birthday"]
        end
      end
    ...
    unless contact.Date_Of_Hire__c.nil?
      if Date.parse(contact.Date_Of_Hire__c).month == today.month
        if special_days.key(Date.parse(contact.Date_Of_Hire__c))
          special_days[Date.parse(contact.Date_Of_Hire__c)] << "#{contact.Name} Work Anniversary"
        else
          special_days[Date.parse(contact.Date_Of_Hire__c)] = ["#{contact.Name} Work Anniversary"]
        end
      end
  ...
  send_event('special_days', {items: special_days.values})
